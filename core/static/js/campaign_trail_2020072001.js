...
reader.readAsText(file);			
	...
	function divideElectoralVotesProp(e, t) {
	    ...
	    function s(t, i, l) {
	        ...
	        var d = '        <div class="game_header">        <h2>NEW CAMPAIGN TRAIL</h2>        </div>        <div class="inner_window_w_desc" id="inner_window_4">            <div id="game_options">            <form name="game_type_selection">            <p><h3>선거인단제를 어떻게 하시겠습니까?</h3>            <select name="game_type_id" id="game_type_id">                <option value=1>기본 (승자독식제)</option>                <option value=2>비례</option>            </select>            </p>            </form>            </div>            <div class="description_window_small"                 id="opponent_selection_description_window">            </div>            <div id="difficulty_level">            <form name="difficulty_level_selection">            <p><h3>Please choose your difficulty level:</h3>            <select name="difficulty_level_id" id="difficulty_level_id">' + o + '</select>            </p>            </form>            </div>        <p id="opponent_selection_id_button_p">        <button class="person_button" id="opponent_selection_id_button">Continue</button>        </p>        </div>';
	        $("#game_window").html(d), $("#game_type_id").ready(function() {
	            ...
	            $("#opponent_selection_id_button").replaceWith("<em>잠시 기다려 주세요...</em>");
	    ...
	    $("#game_start").click(function(a) {
	        a.preventDefault(),
	            function() {
	                ...
	                var l = '<div class="game_header">            <h2>NEW CAMPAIGN TRAIL</h2>        </div>        <div class="inner_window_w_desc" id="inner_window_2">            <div id="election_year_form">            <form name="election_year">            <p>                <h3>몇년도 선거를 진행하실 것인지 선택해주세요:</h3>    \t\t    <select name="election_id" id="election_id">' + a + '</select>            </p>            </form>            <div class="election_description_window" id="election_description_window">                <div id="election_image">                    <img src="' + e.election_json[0].fields.image_url + '" width="300" height="160"/>                </div>                <div id="election_summary">' + e.election_json[0].fields.summary + '</div>            </div>        </div>        <p><button id="election_id_button">계속하기</button></p> <p>이 시나리오는 ' + e.credits + '에 의해 만들어졌습니다.</p>';
